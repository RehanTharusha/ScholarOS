 ...
 */
export function emitComposioEvent(event: {
  toolkitSlug: string;
  success: boolean;
  error?: string;
}): void {
...
export async function initiateConnection(toolkitSlug: string): Promise<{
  ...
  try {
    ...
    const authConfigs = await composioClient.listAuthConfigs(
      toolkitSlug,
      null,
      true,
    );
    ...
    const redirectUrl =
      typeof connectionVal === "object" &&
      connectionVal !== null &&
      "redirectUrl" in connectionVal
    ...
    const { server } = await createAuthServer(8081, async () => {
      ...
      try {
        const accountStatus =
          await composioClient.getConnectedAccount(connectedAccountId);
    ...
    const cleanupTimeout = setTimeout(
      () => {
        if (activeFlows.has(toolkitSlug)) {
          console.log(
            `[Composio] Cleaning up abandoned flow for ${toolkitSlug}`,
          );
          activeFlows.delete(toolkitSlug);
          server.close();
          emitComposioEvent({
...
export async function listToolkits() {
  ...
  const filtered = allItems.filter((item) =>
    CURATED_TOOLKIT_SLUGS.has(item.slug),
  );
  return {
    items: filtered,
