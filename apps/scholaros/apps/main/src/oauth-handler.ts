...
function cancelActiveFlow(reason: string = "cancelled"): void {
  ...
  console.log(
    `[OAuth] Cancelling active flow for ${activeFlow.provider}: ${reason}`,
  );
...
async function getProviderConfiguration(
  ...
  const resolveClientCredentials = async (): Promise<{
    ...
    throw new Error(
      `${provider} client ID not configured. Please provide a client ID.`,
    );
  };
  ...
  if (config.discovery.mode === "issuer") {
    if (config.client.mode === "static") {
      ...
      console.log(
        `[OAuth] ${provider}: Discovery from issuer with static client ID`,
      );
      ...
      const existingRegistration =
        await clientRepo.getClientRegistration(provider);
      ...
      const { config: oauthConfig, registration } =
...
export async function connectProvider(
  ...
  try {
    ...
    const { verifier: codeVerifier, challenge: codeChallenge } =
      await oauthClient.generatePKCE();
    ...
    const { server } = await createAuthServer(8080, async (callbackUrl) => {
      ...
      try {
        ...
        console.log(
          `[OAuth] Exchanging authorization code for tokens (${provider})...`,
        );
    ...
    const cleanupTimeout = setTimeout(
      () => {
        if (activeFlow?.state === state) {
          console.log(
            `[OAuth] Cleaning up abandoned OAuth flow for ${provider} (timeout)`,
          );
