...
export class FSRunsRepo implements IRunsRepo {
  ...
  async fetch(id: string): Promise<z.infer<typeof Run>> {
    ...
    const defaults =
      !rawStart.model || !rawStart.provider
    ...
    const events: z.infer<typeof RunEvent>[] = [
      start,
      ...(rawEvents.slice(1) as z.infer<typeof RunEvent>[]),
    ];
  ...
  async list(cursor?: string): Promise<z.infer<typeof ListRunsResponse>> {
    ...
    if (cursorFile) {
      ...
      if (exact >= 0) {
        ...
        const firstOlder = files.findIndex(
          (name) => name.localeCompare(cursorFile) < 0,
        );
  ...
  async delete(id: string): Promise<void> {
    const filePath = path.join(getScholarOSPath("runs"), `${id}.jsonl`);
  ...
  async deleteAll(): Promise<void> {
    ...
    await Promise.all(
      files.map((f) => fsp.unlink(path.join(runsDir, f)).catch(() => {})),
    );
  }
}
