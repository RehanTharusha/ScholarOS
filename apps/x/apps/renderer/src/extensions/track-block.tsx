...
function TrackBlockView({
  ...
  const triggerType: "scheduled" | "event" | "manual" = schedule
    ? "scheduled"
    : eventMatchCriteria
      ? "event"
      : "manual";
  ...
  const runState = allTrackStatus.get(`${track?.trackId}:${trackFilePath}`) ?? {
  ...
  const handleOpen = (e: React.MouseEvent) => {
    ...
    window.dispatchEvent(
      new CustomEvent<OpenTrackModalDetail>("rowboat:open-track-modal", {
        detail,
      }),
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {instruction && (
          <span className="track-block-chip-instruction">
            {truncate(instruction, 80)}
          </span>
        )}
        {!active && (
          <span className="track-block-chip-paused-label">paused</span>
        )}
      </button>
    </NodeViewWrapper>
  );
...
export const TrackBlockExtension = Node.create({
  ...
  renderHTML({ HTMLAttributes }: { HTMLAttributes: Record<string, unknown> }) {
    return [
      "div",
      mergeAttributes(HTMLAttributes, { "data-type": "track-block" }),
    ];
  },
