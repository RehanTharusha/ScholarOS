...
function FixedSidebarToggle({ leftInsetPx }: { leftInsetPx: number }) {
  ...
  return (
    <div
      className="fixed left-0 top-0 z-50 flex h-10 items-center"
...
function App() {
  ...
  const [paletteContext, setPaletteContext] =
    useState<CommandPaletteContext | null>(null);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === "k") {
        ...
        const handle = activeId
          ? editorRefsByTabId.current.get(activeId)
          : null;
        ...
        <div className="flex h-svh w-full overflow-hidden">
          ...
          >
            <SidebarContentPanel
              ...
              tasksActions={{
                ...
                onSelectRun: (runIdToLoad) => {
                  ...
                  const existingTab = chatTabs.find(
                    (t) => t.runId === runIdToLoad,
                  );
                  if (existingTab) {
                ...
                onDeleteRun: async (runIdToDelete) => {
                  try {
                    await window.ipc.invoke("runs:delete", {
                      runId: runIdToDelete,
                    });
                    // Close any chat tab showing the deleted run
                    const tabForRun = chatTabs.find(
                      (t) => t.runId === runIdToDelete,
                    );
                    if (tabForRun) {
            ...
            <SidebarInset
              ...
              style={
                shouldCollapseLeftPane ? { maxWidth: 0 } : { maxWidth: "100%" }
              }
