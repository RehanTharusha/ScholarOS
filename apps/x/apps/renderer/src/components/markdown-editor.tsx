...
const tableRenderHelpers: MarkdownRendererHelpers = {
  renderChildren: (nodes) => {
    ...
    return arr
      .map((n) => (n.type === "paragraph" ? nodeToText(n as JsonNode) : ""))
      .join("");
  },
...
interface MarkdownEditorProps {
  ...
  onHistoryHandlersChange?: (
    handlers: { undo: () => boolean; redo: () => boolean } | null,
  ) => void;
  editable?: boolean;
...
const createSelectionHighlightExtension = (
  getRange: () => SelectionHighlightRange,
) => {
...
>(function MarkdownEditor(
  ...
  const [atAnchorPosition, setAtAnchorPosition] = useState<{
    left: number;
    top: number;
  } | null>(null);
  const [atCommandValue, setAtCommandValue] = useState<string>("");
  const atKeyStateRef = useRef<{
    open: boolean;
    options: string[];
    value: string;
  }>({ open: false, options: [], value: "" });
  ...
  const maybeCommitPrimaryHeading = useCallback((view: EditorView) => {
    ...
    if (!firstNode || !["heading", "paragraph"].includes(firstNode.type.name))
      return;
  ...
  const preventTitleHeadingDemotion = useCallback(
    (view: EditorView, event: KeyboardEvent) => {
      ...
      const headingLevel =
        ($from.parent.attrs as { level?: number } | null | undefined)?.level ??
        0;
  ...
  const promoteFirstParagraphToTitleHeading = useCallback(
    (view: EditorView) => {
      ...
      const tr = state.tr.setNodeMarkup($from.before(1), headingType, {
        level: 1,
      });
      dispatch(tr);
    },
        ...
        handleKeyDown: (view, event) => {
          ...
          if (state.open) {
            ...
            if (event.key === "ArrowDown" || event.key === "ArrowUp") {
              ...
              const currentIndex = Math.max(
                0,
                state.options.indexOf(state.value),
              );
              const delta = event.key === "ArrowDown" ? 1 : -1;
              const nextIndex =
                (currentIndex + delta + state.options.length) %
          ...
          if (atState.open) {
            ...
            if (event.key === "ArrowDown" || event.key === "ArrowUp") {
              ...
              const currentIndex = Math.max(
                0,
                atState.options.indexOf(atState.value),
              );
              const delta = event.key === "ArrowDown" ? 1 : -1;
              const nextIndex =
                (currentIndex + delta + atState.options.length) %
                atState.options.length;
  ...
  const updateWikiLinkState = useCallback(() => {
    ...
    const range = {
      from: selection.from - matchText.length,
      to: selection.from,
    };
  ...
  const filteredAtOptions = useMemo(() => {
    ...
    return atMentionOptions.filter((opt) =>
