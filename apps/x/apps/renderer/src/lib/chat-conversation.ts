...
export const isChatMessage = (item: ConversationItem): item is ChatMessage =>
  "role" in item;
export const isToolCall = (item: ConversationItem): item is ToolCall =>
  "name" in item;
...
export const getWebSearchCardData = (
  tool: ToolCall,
): WebSearchCardData | null => {
  if (tool.name === "web-search") {
    const input = normalizeToolInput(tool.input) as
    ...
    const mapped = rawResults.map((entry) => ({
      ...
      description:
        entry.description ||
        entry.highlights?.[0] ||
        (entry.text ? entry.text.slice(0, 200) : ""),
    }));
...
const summarizeFilterUpdates = (updates: Record<string, unknown>): string => {
  ...
  if (filters) {
    ...
    const set = filters.set as
      | Array<{ category: string; value: string }>
      | undefined;
    if (set?.length)
      parts.push(
        `Set ${set.length} filter${set.length !== 1 ? "s" : ""}: ${set.map((f) => `${f.category}=${f.value}`).join(", ")}`,
      );
    const add = filters.add as
      | Array<{ category: string; value: string }>
      | undefined;
    if (add?.length)
      parts.push(`Added ${add.length} filter${add.length !== 1 ? "s" : ""}`);
    const remove = filters.remove as
      | Array<{ category: string; value: string }>
      | undefined;
    if (remove?.length)
      parts.push(
        `Removed ${remove.length} filter${remove.length !== 1 ? "s" : ""}`,
      );
  }
  ...
  if (updates.search !== undefined) {
    parts.push(
      updates.search ? `Searching "${updates.search}"` : "Cleared search",
    );
  }
  ...
  if (columns) {
    ...
    if (set)
      parts.push(`Set ${set.length} column${set.length !== 1 ? "s" : ""}`);
    const add = columns.add as string[] | undefined;
    if (add?.length)
      parts.push(`Added ${add.length} column${add.length !== 1 ? "s" : ""}`);
    const remove = columns.remove as string[] | undefined;
    if (remove?.length)
      parts.push(
        `Removed ${remove.length} column${remove.length !== 1 ? "s" : ""}`,
      );
  }
...
export const getAppActionCardData = (
  tool: ToolCall,
): AppActionCardData | null => {
  ...
  if (!result || !result.success) {
    ...
    switch (action) {
      case "open-note":
        return {
          action,
          label: `Opening ${((input.path as string) || "").split("/").pop()?.replace(/\.md$/, "") || "note"}...`,
        };
      case "open-view":
        return { action, label: `Opening ${input.view} view...` };
      case "update-base-view":
        return { action, label: "Updating view..." };
      case "create-base":
        return { action, label: `Creating "${input.name}"...` };
      case "get-base-state":
        return null; // renders as normal tool block
      default:
        return null;
    }
  }
...
const sanitizeBrowserDescription = (value: string | null): string | null => {
  ...
  const looksLikeCssNoise =
    ...
    /display:|position:|background-color|align-items|justify-content|z-index|var\(--|left:|top:/i.test(
      text,
    ) ||
  ...
  if (looksLikeCssNoise || text.length > 88) {
    const quoted = Array.from(text.matchAll(/"([^"]+)"/g))
      ...
      .find(
        (candidate) =>
          candidate &&
          !/display:|position:|background-color|var\(--/i.test(candidate),
      );
...
const getBrowserSuccessLabel = (
  ...
  switch (action) {
    ...
    case "new-tab": {
      ...
      const target =
        formatBrowserTarget(pageUrl) || safeBrowserString(input?.target);
    ...
    case "navigate": {
      ...
      const target =
        formatBrowserTarget(pageUrl) ||
        formatBrowserTarget(safeBrowserString(input?.target));
    ...
    case "click": {
      ...
      if (typeof input?.index === "number")
        return `Clicked element ${input.index}`;
    ...
    case "type": {
      ...
      if (typeof input?.index === "number")
        return `Typed into element ${input.index}`;
    ...
    default:
      return resultMessage
        ? truncateLabel(resultMessage, 72)
        : "Controlled browser";
  }
};
...
export const getBrowserControlLabel = (tool: ToolCall): string | null => {
  ...
  if (result?.success === false) {
    ...
    return error
      ? `Browser error: ${truncateLabel(error, 84)}`
...
export const parseAttachedFiles = (
  content: string,
): { message: string; files: string[] } => {
  const attachedFilesRegex =
    /<attached-files>\s*([\s\S]*?)\s*<\/attached-files>/;
...
export const getComposioConnectCardData = (
  tool: ToolCall,
): ComposioConnectCardData | null => {
...
export const getComposioActionCardData = (
  tool: ToolCall,
): ComposioActionCardData | null => {
  const input = normalizeToolInput(tool.input) as
    | Record<string, unknown>
    | undefined;
  const result = tool.result as Record<string, unknown> | undefined;
  ...
  if (tool.name === "composio-search-tools") {
    ...
    const toolkit = toolkitSlug
      ? COMPOSIO_DISPLAY_NAMES[toolkitSlug] || toolkitSlug
      : null;
    ...
    if (count !== null && tool.status === "completed") {
      label =
        count > 0
          ? `Found ${count} tool${count !== 1 ? "s" : ""} for "${query}"`
  ...
  if (tool.name === "composio-execute-tool") {
    ...
    if (tool.status === "completed") {
      label =
        successful === false ? `Failed: ${readableName}` : `${readableName}`;
      if (toolkit) label += ` on ${toolkit}`;
    }
