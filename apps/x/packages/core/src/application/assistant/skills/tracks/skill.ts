...
Track creation and editing are **action-first**. When the user asks to track, monitor, watch, or pin auto-updating content, you proceed directly — read the file, construct the block, ` +
  "`" +
  `workspace-edit` +
  "`" +
  ` it in. Do not ask "Should I make edits directly, or show you changes first for approval?" — that prompt belongs to generic document editing, not to tracks.
...
- A YAML-fenced ` +
  ...
  `track` +
  "`" +
  ` block that defines the instruction, schedule, and metadata.
...
1. The ` +
  ...
  `track` +
  "`" +
  ` code fence — contains the YAML config. The fence language tag is literally ` +
  "`" +
  `track` +
  "`" +
  `.
2. The target-comment region — ` +
  "`" +
  `<!--track-target:ID-->` +
  "`" +
  ` and ` +
  ...
  `<!--/track-target:ID-->` +
  "`" +
  ` with optional content between. The ID must match the ` +
  "`" +
  `trackId` +
  "`" +
  ` in the YAML.

The target region is **sibling**, not nested. It must **never** live inside the ` +
  "`" +
  "```" +
  `track` +
  "`" +
...
**Runtime-managed fields — never write these yourself:** ` +
  "`" +
  `lastRunAt` +
  "`" +
  `, ` +
...
The schema includes optional ` +
  ...
  `provider` +
  "`" +
  ` fields. **Omit them.** A user-configurable global default already picks the right model and provider for tracks; setting per-track values bypasses that and is almost always wrong.
...
- **Must be unique within the note file.** Before inserting, read the file and check:
  ...
  "`" +
  ` comments
- If you need disambiguation, add scope: ` +
  "`" +
  `btc-price-usd` +
  "`" +
  `, ` +
...
- **Specify output shape.** Describe it concretely: "one line: ` +
  "`" +
  `<temp>°F, <conditions>` +
  "`" +
  `", "3-column markdown table", "bulleted digest of 5 items".
...
If you want consistent style across runs, **describe the style inline** (e.g. "a 3-column markdown table with headers ` +
  "`" +
  `Location` +
  "`" +
  `, ` +
  ...
  `Offset` +
  "`" +
  `"; "a one-line status: HH:MM, conditions, temp"). The track agent only sees your instruction — not this chat, not what you produced last time.
...
- Good: "Fetch USD/INR. Return one line: ` +
  "`" +
  `USD/INR: <rate> (as of <HH:MM IST>)` +
  "`" +
  `."
- Bad: "Give me a nice update about the dollar rate."

**2. Compact table.**
- Good: "Show current local time for India, Chicago, Indianapolis as a 3-column markdown table: ` +
  "`" +
  `Location | Local Time | Offset vs India` +
  "`" +
  `. One row per location, no prose."
- Bad: "Show a polished, table-first world clock with a pleasant layout."

**3. Rolling digest.**
- Good: "Summarize the top 5 HN front-page stories as bullets: ` +
  "`" +
...
- Good: "Check https://status.example.com. Return one line: ` +
  "`" +
  ...
  `⚠ <component>: <status>` +
  "`" +
  `. If degraded, add one bullet per affected component."
...
## YAML String Style (critical — read before writing any ` +
  "`" +
  `instruction` +
  "`" +
  ` or ` +
...
The two free-form fields — ` +
  ...
  `eventMatchCriteria` +
  "`" +
  ` — are where YAML parsing usually breaks. The runner re-emits the full YAML block every time it writes ` +
  "`" +
  `lastRunAt` +
  "`" +
  `, ` +
  ...
  `lastRunSummary` +
  "`" +
  `, etc., and the YAML library may re-flow long plain (unquoted) strings onto multiple lines. Once that happens, any ` +
  "`" +
  `:` +
  "`" +
  ` **followed by a space** inside the value silently corrupts the block: YAML interprets the ` +
  "`" +
  `:` +
  "`" +
  ` as a new key/value separator and the instruction gets truncated.

Real failure seen in the wild — an instruction containing the phrase ` +
  "`" +
  `"polished UI style as before: clean, compact..."` +
  "`" +
  ` was written as a plain scalar, got re-emitted across multiple lines on the next run, and the ` +
  "`" +
  `as before:` +
  "`" +
  ` became a phantom key. The block parsed as garbage after that.

### The rule: always use a safe scalar style

**Default to the literal block scalar (` +
  "`" +
  `|` +
  "`" +
  `) for ` +
  ...
  `eventMatchCriteria` +
  "`" +
  `, every time.** It is the only style that is robust across the full range of punctuation these fields typically contain, and it is safe even if the content later grows to multiple lines.
...
- ` +
  ...
  `|` +
  "`" +
  ` preserves line breaks verbatim. Colons, ` +
  "`" +
  `#` +
  "`" +
  `, quotes, leading ` +
  ...
  `-` +
  "`" +
  `, percent signs — all literal. No escaping needed.
- **Indent every content line by 2 spaces** relative to the key (` +
  "`" +
  `instruction:` +
  "`" +
  `). Use spaces, never tabs.
- Leave a real newline after ` +
  "`" +
  `|` +
  "`" +
  ` — content starts on the next line, not the same line.
- Default chomping (no modifier) is fine. Do **not** add ` +
  "`" +
  `-` +
  "`" +
  ` or ` +
...
- A ` +
  ...
  `|` +
  "`" +
  ` block is terminated by a line indented less than the content — typically the next sibling key (` +
  "`" +
  `active:` +
  "`" +
  `, ` +
...
- Prefer ` +
  ...
  `|` +
  "`" +
  ` the moment the string needs two sentences or a newline.
...
- A literal single quote is escaped by doubling it: ` +
  "`" +
  `'it''s fine'` +
  "`" +
  `.
...
Even if the current value looks safe, a future edit (by you or the user) may introduce a ` +
  "`" +
  `:` +
  "`" +
  ` or ` +
  ...
  `#` +
  "`" +
  `, and a future re-emit may fold the line. The ` +
  "`" +
  `|` +
  "`" +
  ` style is safe under **all** future edits — plain scalars are not.
...
If you ` +
  ...
  `eventMatchCriteria` +
  "`" +
  ` and find it is still a plain scalar, **upgrade it to ` +
  "`" +
  `|` +
  "`" +
  `** in the same edit. Don't leave a plain scalar behind that the next run will corrupt.
...
` +
  ...
  `lastRunSummary` +
  "`" +
  ` are owned by the runner. Don't touch them — don't even try to style them. If your ` +
  "`" +
  `workspace-edit` +
  "`" +
  `'s ` +
  ...
  `oldString` +
  "`" +
  ` happens to include these lines, copy them byte-for-byte into ` +
  "`" +
  `newString` +
  "`" +
  ` unchanged.
...
### ` +
  ...
  `window` +
  "`" +
  ` — recurring within a time-of-day range
...
Fires **at most once per cron occurrence**, but only if the current time is within ` +
  "`" +
  `startTime` +
  "`" +
  `–` +
  ...
  `endTime` +
  "`" +
  ` (24-hour HH:MM, local). Use when the user wants "sometime in the morning" or "once per weekday during work hours" — flexible timing with bounds.

### ` +
  "`" +
  `once` +
  "`" +
  ` — one-shot at a future time
...
**Omit ` +
  ...
  `schedule` +
  "`" +
  ` entirely for a manual-only track** — the user triggers it via the Play button in the UI.

## Event Triggers (third trigger type)

In addition to manual and scheduled, a track can be triggered by **events** — incoming signals from the user's data sources (currently: gmail emails). Set ` +
  "`" +
  `eventMatchCriteria` +
  "`" +
  ` to a description of what kinds of events should consider this track for an update:
...
How it works:
1. When a new event arrives (e.g. an email syncs), a fast LLM classifier checks ` +
  "`" +
  `eventMatchCriteria` +
  "`" +
  ` against the event content.
...
- Mention to the user: scheduled (cron) is for time-driven updates; event is for signal-driven updates. They can be combined — a track can have both a ` +
  "`" +
  `schedule` +
  "`" +
  ` and ` +
  ...
  `eventMatchCriteria` +
  "`" +
  ` (it'll run on schedule AND on relevant events).
...
- Examples: ` +
  "`" +
  `"Emails from John about the migration project"` +
  "`" +
  `, ` +
  "`" +
  `"Calendar events related to customer interviews"` +
  "`" +
  `, ` +
  "`" +
  `"Meeting notes that mention pricing changes"` +
  "`" +
  `.
...
Tracks **without** ` +
  ...
  `eventMatchCriteria` +
  "`" +
  ` opt out of events entirely — they'll only run on schedule or manually.
...
> - notes.md (text/markdown) at notes.md (line 42)
...
3. Check existing ` +
  ...
  `trackId` +
  "`" +
  `s in the file to guarantee uniqueness.
...
6. ` +
  "`" +
  `workspace-edit({ path, oldString: <anchor>, newString: <anchor with block spliced at line N> })` +
  "`" +
  `.
...
3. **Default placement: append** to the end of the file. Find the last non-empty line as the anchor. ` +
  "`" +
  `newString` +
  "`" +
  ` = that line + ` +
...
- a title, category, description, and target folder such as ` +
  "`" +
  `knowledge/Topics/` +
  "`" +
  ` or ` +
...
7. If the target folder is one of the structured knowledge folders (` +
  "`" +
  `knowledge/People/` +
  "`" +
  `, ` +
  ...
  `knowledge/Topics/` +
  "`" +
  `), mirror the local note style by quickly checking a nearby note or config before writing if needed.
...
**Rules:**
- One blank line between the closing ` +
  "`" +
  "```" +
  `" + " fence and the ` +
  "`" +
...
- **Always use the literal block scalar (` +
  "`" +
  `|` +
  "`" +
  `)** for ` +
  ...
  `eventMatchCriteria` +
  "`" +
  `, indented 2 spaces. Never a plain (unquoted) scalar — see the YAML String Style section above for why.
...
- Confirm in one line: "Added ` +
  "`" +
  `chicago-time` +
  "`" +
  ` track, refreshing hourly."
...
- **Do not** write anything into the ` +
  "`" +
  `<!--track-target:...-->` +
  "`" +
  ` region yourself — use the ` +
  "`" +
  `run-track-block` +
  "`" +
  ` tool to delegate to the track agent.
...
The ` +
  ...
  `run-track-block` +
  "`" +
  ` tool manually triggers a track run right now. Equivalent to the user clicking the Play button — but you can pass extra ` +
  "`" +
  `context` +
  "`" +
  ` to bias what the track agent does on this single run (without modifying the block's ` +
  "`" +
  `instruction` +
  "`" +
  `).
...
- **Just created a new track block.** Before declaring done, offer:
  ...
  This is **especially valuable for event-triggered tracks** (with ` +
  "`" +
  `eventMatchCriteria` +
  "`" +
  `) — otherwise the target region stays empty until the next matching event arrives.
...
### Using the ` +
  ...
  `context` +
  "`" +
  ` parameter (the powerful case)

The ` +
  "`" +
  `context` +
  "`" +
  ` parameter is extra guidance for the track agent on this run only. It's the difference between a stock refresh and a smart backfill.

**Examples:**

- New track: "Track emails about Q3 planning" → after creating it, run with:
  > context: "Initial backfill — scan ` +
  "`" +
  `gmail_sync/` +
  "`" +
  ` for emails from the last 90 days that match this track's topic (Q3 planning, OKRs, roadmap), and synthesize the initial summary."

- New track: "Summarize this week's customer calls" → run with:
  > context: "Backfill from this week's meeting notes in ` +
  "`" +
  `granola_sync/` +
  "`" +
  ` and ` +
...
- Plain refresh (user says "run it now"): **omit ` +
  "`" +
  `context` +
  "`" +
  ` entirely**. Don't invent context — it can mislead the agent.

### What to do with the result

The tool returns ` +
  "`" +
  `{ success, runId, action, summary, contentAfter, error }` +
  "`" +
  `:
...
- **` +
  ...
  `action: 'replace'` +
  "`" +
  `** → the track was updated. Confirm with one line, optionally citing the first line of ` +
  "`" +
  `contentAfter` +
  "`" +
  `:
...
- **` +
  ...
  `action: 'no_update'` +
  "`" +
  `** → the agent decided nothing needed to change. Tell the user briefly; ` +
  "`" +
  `summary` +
  "`" +
  ` may explain why.
...
- **` +
  ...
  `error` +
  "`" +
  ` set** → surface it concisely. If the error is ` +
  "`" +
  `'Already running'` +
  "`" +
  ` (concurrency guard), let the user know the track is mid-run and to retry shortly.
...
- **Don't pass ` +
  ...
  `context` +
  "`" +
  `** for a plain refresh — only when there's specific extra guidance to give.
- **Don't use ` +
  "`" +
  `run-track-block` +
  "`" +
  ` to manually write content** — that's ` +
  "`" +
  `update-track-content` +
  "`" +
  `'s job (and even that should be rare; the track agent handles content via this tool).
- **Don't ` +
  "`" +
  `run-track-block` +
  "`" +
  ` repeatedly** in a single turn — one run per user-facing action.
...
- **Don't add ` +
  ...
  `schedule` +
  "`" +
  `** if the user explicitly wants a manual-only track.
- **Don't write** ` +
  "`" +
  `lastRunAt` +
  "`" +
...
- **Don't touch** content between ` +
  ...
  `<!--/track-target:ID-->` +
  "`" +
  ` — that's generated content.
- **Don't schedule** with ` +
  "`" +
  `"* * * * *"` +
  "`" +
  ` (every minute) unless the user explicitly asks.
- **Don't add a ` +
  "`" +
  `Z` +
  "`" +
...
- **Don't use ` +
  ...
  `workspace-writeFile` +
  "`" +
  `** to rewrite the whole file — always ` +
  "`" +
  `workspace-edit` +
  "`" +
...
**Change schedule or instruction:** read the file, ` +
  "`" +
  `workspace-edit` +
  "`" +
  ` the YAML body. Anchor on the unique ` +
  "`" +
  `trackId: <id>` +
  "`" +
  ` line plus a few surrounding lines.

**Pause without deleting:** flip ` +
  "`" +
  `active: false` +
  "`" +
  `.
...
**Remove entirely:** ` +
  ...
  `track` +
  "`" +
  ` block **plus** the target pair (so generated content also disappears), ` +
  "`" +
  `newString` +
  "`" +
  ` = empty.
...
YAML style reminder: ` +
  ...
  `|` +
  "`" +
  ` block scalars. Never plain. Never leave a plain scalar in place when editing.
`;
