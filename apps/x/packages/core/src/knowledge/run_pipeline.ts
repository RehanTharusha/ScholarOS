...
async function main() {
  ...
  if (
    (steps.includes("label") || steps.includes("graph")) &&
    !fs.existsSync(gmailSyncDir)
  ) {
    console.warn(
      `[Pipeline] Warning: gmail_sync/ folder not found in ${workdir}`,
    );
  }
  ...
  if (steps.includes("label")) {
    ...
    const labelEmailsModulePath: string = "./label_emails.js";
    try {
      const { processUnlabeledEmails } = await import(labelEmailsModulePath);
      await processUnlabeledEmails(concurrency);
    } catch (error) {
      console.warn(
        "[Pipeline] Email labeling step unavailable, skipping:",
  ...
  if (fs.existsSync(knowledgeDir)) {
    ...
    console.log(
      `[Pipeline] Output: ${countFiles(knowledgeDir)} notes in ${knowledgeDir}`,
    );
  }
}
