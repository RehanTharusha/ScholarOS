...
import { GetDocumentsResponse, SyncState, Document } from "./types.js";
  ...
  while (retries < MAX_RETRIES) {
    try {
      ...
      if (
        ...
        retries++;
        console.log(
          `[Granola] Rate limit hit for ${operationName}. Retry ${retries}/${MAX_RETRIES} in ${delay / 1000}s...`,
        );

        if (retries >= MAX_RETRIES) {
          console.error(
            `[Granola] Max retries reached for ${operationName}. Skipping.`,
          );
          return null;
        }
...
async function apiCall<T>(
  ...
  if (!response.ok) {
    ...
    console.error(
      `[Granola] API error ${response.status}: ${response.statusText} - ${errorText.slice(0, 200)}`,
    );
...
async function getDocuments(
  ...
  try {
    ...
    console.log(
      `[Granola] Fetched ${parsed.docs.length} documents (offset: ${offset})`,
    );
    return parsed;
  } catch (error) {
    console.error("[Granola] Failed to parse documents response:", error);
    console.error(
      "[Granola] Raw response:",
...
function convertProseMirrorToMarkdown(
...
function documentToMarkdown(doc: Document): string {
  ...
  if (
    lastViewedContent &&
    typeof lastViewedContent === "object" &&
    lastViewedContent.type === "doc"
  ) {
...
async function syncNotes(): Promise<void> {
  ...
  try {
    // Check if enabled
    const granolaRepo =
      container.resolve<IGranolaConfigRepo>("granolaConfigRepo");
    ...
    while (hasMore) {
      ...
      for (const doc of docsResponse.docs) {
        ...
        if (docDate < lookbackCutoff) {
          console.log(
            `[Granola] Document "${doc.title}" is older than ${LOOKBACK_DAYS} days, stopping pagination`,
          );
          foundOldDoc = true;
          break;
        }
    ...
    console.log(
      `[Granola] Sync complete: ${newCount} new, ${updatedCount} updated`,
    );
    ...
    if (runId) {
      ...
      await serviceLogger.log({
        ...
        items: limitedTitles,
        truncated: changedTitles.length > limitedTitles.length,
      });
      await serviceLogger.log({
        type: "run_complete",
        service: "granola",
...
export async function init(): Promise<void> {
  ...
  while (true) {
    ...
    console.log(
      `[Granola] Sleeping for ${SYNC_INTERVAL_MS / 60000} minutes...`,
    );
