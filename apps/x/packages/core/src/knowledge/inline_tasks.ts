...
function extractAllFrontmatterValues(
  raw: string | null,
      ...
      return {
        ...
        schedule: parsed.data.schedule
          ? ({
              ...
              label: parsed.data["schedule-label"] ?? "",
            } as InlineTaskSchedule)
          : null,
  ...
  for (const cl of contentLines) {
    ...
    if (schedMatch) {
      try {
        ...
        /* not JSON schedule, skip */
      }
    } else if (!/^schedule-config:\s/.test(cl.trim())) {
      instructionLines.push(cl);
    }
  }
  const firstRowboatLine = instructionLines.find((l) =>
    l.trim().startsWith("@rowboat"),
  );
  const rawInstruction =
    firstRowboatLine?.trim() ?? instructionLines.join("\n").trim();
 ...
 */
function isScheduledTaskDue(
  schedule: InlineTaskSchedule,
  lastRunAt: string | null,
): boolean {
  const now = new Date();
...
function findPendingTasks(body: string): InlineTask[] {
  ...
  while (i < lines.length) {
    ...
    if (
      ...
      trimmed.startsWith("```tell-rowboat")
    ) {
      const startLine = i;
      i++;
...
async function processInlineTasks(): Promise<void> {
  ...
  for (const filePath of allFiles) {
    ...
    console.log(
      `[InlineTasks] Found ${tasks.length} pending task(s) in ${relativePath}`,
    );
    ...
    for (const task of sortedTasks) {
      console.log(
        `[InlineTasks] Running task: "${task.instruction.slice(0, 80)}..."`,
      );
      ...
      try {
        ...
        if (result) {
          if (task.targetId) {
            ...
            currentBody = replaceTargetRegion(
              currentBody,
              task.targetId,
              result,
              task.endLine,
 ...
 */
export async function classifySchedule(
  instruction: string,
): Promise<InlineTaskSchedule | null> {
  ...
  const localEnd = defaultEnd.toLocaleString("en-US", {
    dateStyle: "full",
    timeStyle: "long",
  });
...
export async function init() {
  ...
  console.log(
    `[InlineTasks] Will check for task blocks every ${SYNC_INTERVAL_MS / 1000} seconds`,
  );
