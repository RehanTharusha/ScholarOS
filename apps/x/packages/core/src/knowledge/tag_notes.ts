...
export async function processUntaggedNotes(): Promise<void> {
  ...
  for (let i = 0; i < untagged.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      console.log(
        `[NoteTagging] Processing batch ${batchNumber}/${totalBatches} (${files.length} files)`,
      );
      await serviceLogger.log({
        type: "progress",
        service: run.service,
      ...
      console.log(
        `[NoteTagging] Batch ${batchNumber}/${totalBatches} complete, ${result.filesEdited.size} files tagged`,
      );
    } catch (error) {
      hadError = true;
      console.error(
        `[NoteTagging] Error processing batch ${batchNumber}:`,
        error,
      );
      await serviceLogger.log({
        type: "error",
