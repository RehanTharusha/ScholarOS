    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          if (
            fromLines?.some((line) =>
              line.toLowerCase().includes(userEmailLower),
            )
          ) {
...
function extractUserPartsFromEmail(
  content: string,
  userEmail: string,
): string | null {
  const userEmailLower = userEmail.toLowerCase();
  ...
  try {
    if (await useComposioForGoogle()) {
      ...
      if (account && account.status === "ACTIVE") {
        ...
        const email = (result.data as Record<string, unknown>)?.emailAddress as
          | string
          | undefined;
        if (email) {
          updateUserEmail(email);
          console.log(
            `[AgentNotes] Auto-populated user email via Composio: ${email}`,
          );
          return email;
        }
      }
    ...
    console.log(
      "[AgentNotes] Could not fetch email via Composio:",
