...
export class FSAgentsRepo implements IAgentsRepo {
  private readonly agentsDir = getScholarOSPath("agents");
  ...
  async list(): Promise<z.infer<typeof Agent>[]> {
    ...
    for (const file of matches) {
      try {
        ...
        console.error(
          `Error parsing agent ${file}: ${error instanceof Error ? error.message : String(error)}`,
        );
        continue;
      }
    }
  ...
  private async parseAgentMd(filepath: string): Promise<z.infer<typeof Agent>> {
    ...
    if (frontmatter) {
      const parsed = Agent.omit({ instructions: true }).parse(frontmatter);
  ...
  async fetch(id: string): Promise<z.infer<typeof Agent>> {
    const agent = await this.parseAgentMd(
      path.join(this.agentsDir, `${id}.md`),
    );
    return {
      ...agent,
      name: id,
  ...
  async update(
    id: string,
    agent: z.infer<typeof UpdateAgentSchema>,
  ): Promise<void> {
    const { instructions, ...rest } = agent;
    const contents = `---\n${stringify(rest)}\n---\n${instructions}`;
