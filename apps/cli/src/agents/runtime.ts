...
import {
  IdGen,
  IMonotonicallyIncreasingIdGenerator,
} from "../application/lib/id-gen.js";
...
function normaliseAskHumanToolCall(message: z.infer<typeof AssistantMessage>) {
  ...
  const newParts = [];
  for (const part of message.content as z.infer<
    typeof AssistantContentPart
  >[]) {
...
export async function* streamAgent({
  ...
  const model = provider.languageModel(
    agent.model || modelConfig.defaults.model,
  );
